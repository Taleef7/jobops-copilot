...
- [x] #252 **get_model_for_agent(agent_id): hot-swappable per-agent models in provider.py (per-(agent,version) cache, env fallback)** (M; blocked by: #251)
- [x] #253 **Four-graph registry + POST /agents/{agent_id}/stream|resume (SSE) + apps/api 3-hop proxy** (L; blocked by: #252)
- [x] #254 **Durable AsyncPostgresSaver + AsyncPostgresStore in lifespan, thread/namespace conventions, checkpoint pruning** (L; blocked by: #253) — ⚠️ `RUN_MIGRATIONS_ON_BOOT` does NOT gate this DDL (it is read only by `apps/api`); own an agent-side setup lever and fail closed instead of falling back to the in-memory saver
- [x] #255 **Per-agent Langfuse tags, per-run token budget with hard abort, recursion-limit defaults** (M; blocked by: #253)
- [x] #256 **Assistant chat gains four specialist tools invoking the registry graphs (subagents pattern)** (M; blocked by: #253)
- [x] #257 **langgraph.json for all graphs + local debugging doc (langgraph dev + Studio, LANGSMITH_TRACING=false)** (S; blocked by: #253)
...
- [x] #258 **feat(api): enrich the jobs schema — salary/seniority/sponsorship/liveness columns, salary+seniority parsers, stop discarding Adzuna salary** (L; blocked by: none)
- [x] #259 **feat(api): USAJobs + The Muse source adapters; fix the Remotive fallback window; multi-source discovery** (L; blocked by: #258)
- [x] #260 **feat(api,web): target_companies table + CRUD + settings section to manage the ATS-board watchlist** (M; blocked by: none) — ⚠️ the table needs `user_id` + `unique (user_id, board_type, board_token)`; the spec's original column list omitted it and would have made one shared watchlist for every account
- [x] #261 **feat(api): Greenhouse, Lever, and Ashby public-board adapters wired into discovery via target_companies** (L; blocked by: #258, #260)
...
- [x] #271 **feat(api): email channel adapter, daily digest, and digest cron endpoint** (L; blocked by: #269)
- [x] #272 **feat(web,api): PWA installability + VAPID web-push channel** (L; blocked by: #269)
- [x] #273 **feat(api): emit notification events — job matches, follow-up nudges, approvals** (M; blocked by: #269, #271)
- [x] #274 **feat(web): notifications inbox, header bell, and notification settings UI** (L; blocked by: #269, #272)
